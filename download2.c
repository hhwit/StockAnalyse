...
static void download_stock(char *code, int num, char *path)
{
	...
	memset(cmd, 0, sizeof(cmd));
	sprintf(cmd, "wget -O %s/%s \"https://money.finance.sina.com.cn/quotes_service/api/json_v2.php/CN_MarketData.getKLineData?symbol=%s%s&scale=240&ma=no&datalen=%d\"", path, code, code[0]=='6'?"sh":"sz", code, num);
	//printf("cmd:%s\n", cmd);
	system(cmd);
}
...
int main(int argc, char *argv[])
{
	char path[32];
	if (argc < 3) {
		printf("Please input path & how many days for saving\n");
		return 0;
	}
